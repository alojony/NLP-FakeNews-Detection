...
def build_dataset(true_path, false_path, num_class_samples=-1, rnd_state=42):
    ...
    true_df = true_df[(true_df['text'].str.strip() != "") & (true_df['title'].str.strip() != "")]
    false_df = false_df[(false_df['text'].str.strip() != "") & (false_df['title'].str.strip() != "")]

    if num_class_samples == -1:
        true_sampled_df = true_df
        false_sampled_df = false_df
    else:
        true_sampled_df = true_df.sample(n=min(len(true_df), num_class_samples), replace=False, random_state=rnd_state)
        false_sampled_df = false_df.sample(n=min(len(false_df), num_class_samples), replace=False, random_state=rnd_state)
